...
export async function deployWhizperZKPVerifier(
  contractName = 'WhizperVerifier'
): Promise<Contract> {
  const WhizperVerifier = await ethers.getContractFactory(contractName);
  const whizperVerifier = await upgrades.deployProxy(WhizperVerifier);
  console.log(contractName + ' deployed to:', await whizperVerifier.getAddress());
  return whizperVerifier;
}
